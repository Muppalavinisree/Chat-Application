...
import vini from '../../assets/vini.webp';
				...
				{messages?.receiver?.fullName && (
					<div className="w-[75%] bg-secondary h-[80px] my-14 rounded-full flex items-center px-14 py-2">
						<div className="cursor-pointer">
							<img src={Img1} width={60} height={60} className="rounded-full" alt="" />
						</div>
  ...
  <div className="p-14">
    {messages?.messages?.length > 0 ? (
      messages.messages.map(({ message, user: { id } = {} }) => {
        return (
          <div key={id}> {/* Use a unique key, e.g., user id or message id */}
            <div
								...
								<div key={userId} className="flex items-center py-8 border-b border-b-gray-300">
									...
									>
										<div>
											<img
												src={Img1}
												className="w-[60px] h-[60px] rounded-full p-[2px] border border-primary" alt=""
											/>
										</div>
										<div className="ml-6">
											<h3 className="text-lg font-semibold">{user?.fullName}</h3>
